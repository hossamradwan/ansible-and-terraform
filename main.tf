...
resource "aws_default_security_group" "default-sg" {
  ...
  egress {
    ...
    cidr_blocks = ["0.0.0.0/0"] 
    prefix_list_ids = [] #allow access to vpc endpoint
  }
...
resource "aws_key_pair" "ssh-key" {
  key_name = "terraform-server-key-pair-2"
  public_key = file(var.public_key_location)
}

resource "aws_instance" "myapp-server" {
  ami = data.aws_ami.latest-amazon-linux-image.id #ec2 image id
  #ami = "ami-0fb653ca2d3203ac1"  # id of ubuntu image
